...
def plot(output_dir: Path) -> None:
    ...
    summary_lines = ["Phase 2 Completed Runs (indices 0-20)", "", "Included series:"]
    ...
    for ax, (curve_key, title, ylabel) in zip(axes, panels):
        ...
        ax.set_title(f"Phase 2 Completed (0-20) - {title}")
    ...
    fig.savefig(output_dir / "phase2_completed_0_20.png", bbox_inches="tight")
    fig.savefig(output_dir / "phase2_completed_0_20.pdf", bbox_inches="tight")
    plt.close(fig)

    summary_lines.append("")
    summary_lines.append("Note: this figure uses only seed 0,1,2 and excludes frames=12000 plus later extra seeds.")
    (output_dir / "phase2_completed_0_20_summary.txt").write_text("\n".join(summary_lines) + "\n")


if __name__ == "__main__":
    plot(Path("/home/naikewu/RL/BenchMARL/plots/phase2_completed_0_20"))
