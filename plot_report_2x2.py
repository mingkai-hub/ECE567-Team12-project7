...
OUT = Path("/home/mingkai/BenchMARL/plots/report_2x2_stageA_stageB.png")
OUT_REPORT = Path("/home/mingkai/BenchMARL/plots/report_2x2_stageA_stageB_clean.png")
...
def load_stage_a_runs(task: str, alg: str, completed_extra):
    ...
    for seed_dir in sorted(task_path.glob("seed_*")):
        ...
        if int(seed) >= 3 and (task, alg, seed) not in completed_extra:
            continue
        matches = list(seed_dir.glob("*/*/scalars/eval_reward_episode_reward_mean.csv"))
        if not matches:
            continue
...
def load_stage_b_runs(variant: str, completed_round2):
    ...
    for seed in ["0", "1", "2", "3", "4", "5"]:
        ...
        matches = list(seed_root.glob("*/*/scalars/eval_reward_episode_reward_mean.csv"))
        if not matches:
            continue
        metric_path = matches[0]
        scalars_dir = metric_path.parent
...
def main():
    ...
    completed_round2, _ = load_round2_maps()
    ...
    ax_b.set_ylabel("Evaluation Reward")
    ax_b.set_ylim(-6, 2)
    ...
    fig.suptitle("BenchMARL/IPPO Project Results Overview", fontsize=14)
