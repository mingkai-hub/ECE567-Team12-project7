...
def plot(output_dir: Path) -> None:
    ...
    summary_lines = ["Phase 2 Completed Results", "", "Included series:"]
    ...
    for ax, (curve_key, title, ylabel) in zip(axes, panels):
        ...
        ax.set_title(f"Phase 2 Completed - {title}")
    ...
    fig.savefig(output_dir / "phase2_completed_all.png", bbox_inches="tight")
    fig.savefig(output_dir / "phase2_completed_all.pdf", bbox_inches="tight")
    plt.close(fig)

    summary_lines.append("")
    summary_lines.append(f"Note: only runs with last_frames >= {COMPLETE_FRAMES} are included.")
    (output_dir / "phase2_completed_all_summary.txt").write_text("\n".join(summary_lines) + "\n")


if __name__ == "__main__":
    plot(Path("/home/naikewu/RL/BenchMARL/plots/phase2_completed_all"))
