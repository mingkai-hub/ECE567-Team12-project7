...
def load_completed_runs(seed_root: Path) -> List[dict]:
    runs: List[dict] = []
...
def plot(output_dir: Path) -> None:
    ...
    summary_lines = ["Phase 2 Completed Results (n != 3 only)", "", "Included series:"]

    for ax, (curve_key, title, ylabel) in zip(axes, panels):
        for _, spec in series.items():
            runs = load_completed_runs(spec["path"])
            if len(runs) == 3 or not runs:
                continue
            ...
            if curve_key == "eval_curve":
                ...
                summary_lines.append(f"- {spec['label']}: {len(runs)} seed(s) [{seeds}]")

        ax.set_title(f"Phase 2 Completed (n != 3) - {title}")
    ...
    fig.savefig(output_dir / "phase2_completed_n_not_3.png", bbox_inches="tight")
    fig.savefig(output_dir / "phase2_completed_n_not_3.pdf", bbox_inches="tight")
    plt.close(fig)

    summary_lines.append("")
    summary_lines.append("Note: only completed series with n != 3 are included.")
    (output_dir / "phase2_completed_n_not_3_summary.txt").write_text("\n".join(summary_lines) + "\n")


if __name__ == "__main__":
    plot(Path("/home/naikewu/RL/BenchMARL/plots/phase2_completed_n_not_3"))
